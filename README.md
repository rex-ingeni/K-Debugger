...
**플러그인별 후킹 원리**
- **로그** — 자동. `KDebugger.init()` 후 SDK가 앱 프로세스의 logcat을 스트리밍(연결 중에만)하여 `System.out.println`·`Log.*`·Timber·네이티브 로그를 모두 캡처한다. 로그 Tree/래퍼를 심을 필요 없음(자기 프로세스만 읽으므로 `READ_LOGS` 불요)
